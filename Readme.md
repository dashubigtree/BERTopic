...
結論：目前已經使用data set當中的文章內容進行BERTopic的訓練。並且也將每一篇文章分類的主題結果標示回csv，同時附加該主題的十個關鍵字。

# 0427 record
增加老師提供的新dataset's seed words
- 老師有問到：強制分類。這個主題分類建議，但是這個建議使用的方式就是將「每一個主題只要有兩篇文章即可視為獨立的一個主題」
（另有問所謂強制分類，就是把min_cluster_size設定成2，但對方說可能會出現旺很多主題。)
但是這個設定會讓主題變得非常細分，所以我就先不使用這個設定。

- 有嘗試加入所有初級制度的seed words，但看起來不太影響分群效果。

# 0430 record
今天嘗試去解決一下-1類別的文章數量問題，因為之前發現在分類結果中會出現-1類別的文章數量都大於其他主題。
目前觀察的結果是問題可能出在HDBSCAN（clustering model)上，因為此方法是透過「密度」來決定分群結果。好處是此方法不會受到數據點的分佈形狀而受到限制，相反地，這方法可能會造成離群值的產生。也就是會有一些文章因為不夠緊密而無法被判定成任何一個類別，這也是為什麼-1類別的文章數量都會是最多的原因。

##### 如果希望保留HDBSCAN的優勢但減少離群值：

- **降低min_cluster_size**：減小最小聚類大小，允許更小的聚類形成
- **降低min_samples**：減小用於確定核心點的樣本數
- **調整cluster_selection_epsilon**：增加此值可以擴大聚類邊界，捕獲更多邊緣文檔

# 0503 record
重新整理一下seed words的兩次嘗試，一次是只有初級制度的主題關鍵詞；另一個是所有主題各自延伸的關鍵詞。
1. {"权力平衡", "发展", "外交", "人类平等", "国际法", "民族主义", "主权", "领土性", "市场", "战争", "民主", "环境管理", "人权",}
2. {"权力平衡", "联盟", "平衡", "发展", "社会进步", "人类进步", "物质进步", "外交", "双边会议", "外交的", "使者", "多边", "外交语言", "大会", "人类平等", "种族隔离", "殖民地", "性别平等", "种族的", "种族主义", "种族主义者", "性别", "前领土", "附属地", "海外领地", "殖民地", "国际法", "宪章", "公约", "法院", "法律", "具有法律约束力的文件", "议定书", "条约", "仲裁", "民族主义", "民族主义者", "自决", "民族自决", "人民主权", "主权", "独立", "不干涉", "不干预", "国家责任", "领土性", "边界", "疆界", "领土的", "领土", "市场", "经济的", "经济", "贸易", "经济一体化", "保护主义", "贸易壁垒", "关税", "战争", "使用武力", "进攻", "防御", "侵略", "防卫", "自卫", "民主", "民主的", "议会", "议会的", "少数派", "投票", "选举", "集会自由", "少数派权利", "环境管理", "气候变化", "生态平衡", "环境的", "保护环境", "环境保护", "排放", "全球变暖", "森林砍伐", "海平面", "温室效应", "人权", "酷刑", "言论自由", "奴役", "奴隶制", "监禁", "种族灭绝", "权利",}

我嘗試這兩種seed words的結果並沒有另外儲存到不同的model and visualization folder route。
但是我有將每次的視覺化結果compress成zip 檔案，並且上傳到google drive。
-> 只有初級制度的關鍵詞：MainSeedsWords_MinCluster2_chi_sp_20240119.zip
-> 所有主題各自延伸的關鍵詞：AllSeedsWords_MinCluster2_chi_sp_20240119.zip