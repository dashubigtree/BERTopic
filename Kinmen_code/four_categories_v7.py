...
def preprocess_text(text):
    # 新增數字過濾和詞性標註
    tokens = [word for word, flag in pseg.lcut(text) 
              if flag.startswith('n') and not word.isdigit()]
    
    # # 新增同義詞合併 (這邊參考一下，需要詳細去得知一下有沒有同義字替換的問題)
    # synonym_map = {"陸方":"大陸","台海":"台灣海峽","漁業":"水產"}
    # tokens = [synonym_map.get(token, token) for token in tokens]
    
    # 原有處理流程保持不變
    bigram = Phraser(Phrases([tokens], min_count=5, threshold=10))
    return [token for token in bigram[tokens] if token not in stop_words]
...
#         # 依照句子切割內容
#         sentences = split_by_sentence(row['content'])
...
#                 'title': row['title'],
#                 'date': row['date'],
#                 'content': sentence,  # 使用切割後的句子
#                 'Official media': row['Official media'],
#                 'author': row['author'],
#                 'media': row['media'],
#                 'account': row['account'],
#                 'Internet news license': row['Internet news license'],
#                 'Commercial media': row['Commercial media'],
#                 'kmt': row['kmt']
#             }
...
# # 讀取原始資料
# csv_file_path = "/Users/shuyuhsu/code_workspace/Kinmen_wechat_BERTTopic/Kinmen_code/data/OriginalData_Kinmen_20250221_clr.csv"
...
# # 保存結果
# output_path = "/Users/shuyuhsu/code_workspace/Kinmen_wechat_BERTTopic/Kinmen_code/data/Kinmen_splitData_20250223_sentence_new.csv"
# processed_df.to_csv(output_path, index=False, encoding="utf-8-sig")
# print(f"資料已依照句子切割並保存至: {output_path}")

# 讀取和處理數據的部分保持不變
csv_file_path = "./data/Kinmen_splitData_20250223_sentence_new.csv"
df = pd.read_csv(csv_file_path)
...
# 檢查處理後的資料
print(f"清理後有效資料筆數: {len(df)}")
if len(df) == 0:
    raise ValueError("資料清理後無有效數據，請檢查原始資料或清理規則")
...
# 儲存模型
model_save_path = "./model/bertopic_four_categories_v7"  # 更新路徑
...
# 確保視覺化目錄存在
visualization_path = "./visualization/four_categories_v7"  # 更新路徑
...
def save_topic_results(topic_model, topic_info, texts, df, topics, base_path="./results/four_categories_v7"):
    """整合儲存主題相關資訊"""
    # 確保目錄存在
