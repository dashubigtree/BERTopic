...
# 儲存模型
model_save_path = "./model/bertopic_four_categories_v5"  # 更新路徑
...
# 確保視覺化目錄存在
visualization_path = "./visualization/four_categories_v5"  # 更新路徑
os.makedirs(visualization_path, exist_ok=True)

# 在其他視覺化之前，先進行主題合併
print("\n開始合併相似主題...")
try:
    # 根據層次圖中距離小於0.5的主題進行合併
    topics_to_merge = [
        [11, 5, 6, 4, 1, 2],     # 第一組相似主題
        [13, 8, 3, 7],           # 第二組相似主題
        [21, 19, 15, 12]         # 第三組相似主題
    ]
    ...
    # 保存合併後的模型
    merged_model_path = "./model/bertopic_four_categories_v5_merged"
    topic_model.save(merged_model_path)
    print(f"合併後的模型已保存至: {merged_model_path}")
    
    # 重新生成層次聚類圖
    fig_hierarchical_merged = topic_model.visualize_hierarchy(
        width=1800,
        height=1200,
        title="合併後的主題層次結構圖"
    )
    fig_hierarchical_merged.write_html(f"{visualization_path}/hierarchical_clustering_merged.html")
    
    print("主題合併完成")
    
except Exception as e:
    print(f"合併主題時出錯: {str(e)}")
...
# 設置日誌目錄
log_dir = "./visualization/four_categories_v5/log"
...
try:
    ...
    logging.error(f"層次聚類圖生成失敗: {str(e)}")
    logging.error(f"詳細錯誤信息:\n{traceback.format_exc()}")
...
# 設置日誌目錄
log_dir = "./visualization/four_categories_v5/log"  # 更新路徑
os.makedirs(log_dir, exist_ok=True)
